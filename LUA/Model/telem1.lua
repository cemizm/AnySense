------------------------------------------------------
--  09.05.2015										--
--  Telemetrie Skript fŸr FrSky Taranis				--
--  benštigt AnySense fŸr vollen Funktionsumfang	--
...
-----------------------------------------
-- Definition benutzerspezifische Werte--
-----------------------------------------
local headingHomeLock = 1							-- 1 = ja, 0 = nein

local playHomeSet = 1								-- 1 = ja, 0 = nein
local playMotorArmed = 1							-- 1 = ja, 0 = nein
	...
	if lastMotor ~= data.armed then
		...
		if data.armed then
			...
			if playMotorArmed == 1 then
				playFile("/SCRIPTS/WAV/bldson.wav")
			end
		else
			if playMotorArmed == 1 then
				playFile("/SCRIPTS/WAV/bldsoff.wav")
			end
		end
	...
	if lastHome ~= data.homeSet then
		...
		if data.homeSet then
			...
			if playHomeSet == 1 then
				playFile("/SCRIPTS/WAV/antrfrei.wav")
			end
...
local function run ()
	...
	lcd.drawPixmap(3, 16, "/SCRIPTS/BMP/Frostie/gps.bmp")
	lcd.drawNumber(60, 16, data.sats, DBLSIZE)
	
	if (data.sats > 6) then
		lcd.drawPixmap(18, 16, "/SCRIPTS/BMP/Frostie/gps_5.bmp")
	elseif (data.sats > 5) then
		lcd.drawPixmap(18, 16, "/SCRIPTS/BMP/Frostie/gps_4.bmp")
	elseif (data.sats > 4) then
		lcd.drawPixmap(18, 16, "/SCRIPTS/BMP/Frostie/gps_3.bmp")
	elseif (data.sats > 2) then
		lcd.drawPixmap(18, 16, "/SCRIPTS/BMP/Frostie/gps_2.bmp")
	elseif (data.sats > 0) then
		lcd.drawPixmap(18, 16, "/SCRIPTS/BMP/Frostie/gps_1.bmp")
	elseif data.sats < 1 then
		lcd.drawPixmap(18, 16, "/SCRIPTS/BMP/Frostie/gps_0.bmp")
	end

	lcd.drawPixmap (62,16, "/SCRIPTS/BMP/Frostie/satfix_"..data.satfix..".bmp")
	...
	if data.cell > 0 then
		...
		lcd.drawPixmap (3,35,"/SCRIPTS/BMP/Frostie/Akku-"..fileindex..".bmp")
		...
		lcd.drawPixmap (3,35,"/SCRIPTS/BMP/Frostie/Akku-0.bmp")
	...
	lcd.drawPixmap (80,21,"/SCRIPTS/BMP/Frostie/Pfeil"..fileindex..".bmp")
	...
	if not (long_home == 0 and lat_home == 0) then												--Homepunkt nur anzeigen, wenn er auch gesetzt ist.
		...
		lcd.drawPixmap (89+x,30+y,"/SCRIPTS/BMP/Frostie/Homepunkt.bmp")    						-- Home Kreis zeichnen
	end
	...
	lcd.drawText(125, 24, "Hohe ", 0)
	lcd.drawPoint(132, 24, 1)
	lcd.drawPoint(134, 24, 1)
	lcd.drawText(125, 38, "Geschw.", 0)
	lcd.drawText(125, 52, "Distanz ", 0)
	...
	lcd.drawPixmap(141, 4, "/SCRIPTS/BMP/Frostie/timer.bmp")
