...
namespace MavLink
{
    ...
    public static class MavLinkSerializer
    {
        ...
        {
			{180, new MavPacketInfo(Deserialize_CONFIGURATION_CONTROL, 155)},
		...
		internal static MavlinkMessage Deserialize_CONFIGURATION_CONTROL(byte[] bytes, int offset)
		{
			return new Msg_configuration_control
			{
				param1 = bitconverter.ToUInt16(bytes, offset + 0),
				param2 = bitconverter.ToUInt16(bytes, offset + 2),
				command = bytes[offset + 4],
			};
		}

		internal static MavlinkMessage Deserialize_CONFIGURATION_VERSION(byte[] bytes, int offset)
		{
			return new Msg_configuration_version
			{
				fw_version = bitconverter.ToUInt16(bytes, offset + 0),
				port1 = bytes[offset + 2],
				port2 = bytes[offset + 3],
			};
		}

		internal static MavlinkMessage Deserialize_CONFIGURATION_NAZA_HEARTBEAT(byte[] bytes, int offset)
		{
			return new Msg_configuration_naza_heartbeat
			{
				lastNazaHeartbeat = bitconverter.ToUInt16(bytes, offset + 0),
			};
		}

		internal static MavlinkMessage Deserialize_CONFIGURATION_PORT(byte[] bytes, int offset)
		{
			return new Msg_configuration_port
			{
				port = bytes[offset + 0],
		...
		internal static int Serialize_CONFIGURATION_CONTROL(this Msg_configuration_control msg, byte[] bytes, ref int offset)
		{
			bitconverter.GetBytes(msg.param1, bytes, offset + 0);
			bitconverter.GetBytes(msg.param2, bytes, offset + 2);
			bytes[offset + 4] = msg.command;
			offset += 5;
			return 180;
		}

		internal static int Serialize_CONFIGURATION_VERSION(this Msg_configuration_version msg, byte[] bytes, ref int offset)
		{
			bitconverter.GetBytes(msg.fw_version, bytes, offset + 0);
			bytes[offset + 2] = msg.port1;
			bytes[offset + 3] = msg.port2;
			offset += 4;
			return 181;
		}

		internal static int Serialize_CONFIGURATION_NAZA_HEARTBEAT(this Msg_configuration_naza_heartbeat msg, byte[] bytes, ref int offset)
		{
			bitconverter.GetBytes(msg.lastNazaHeartbeat, bytes, offset + 0);
			offset += 2;
			return 182;
		}

		internal static int Serialize_CONFIGURATION_PORT(this Msg_configuration_port msg, byte[] bytes, ref int offset)
		{
			bytes[offset + 0] = msg.port;
			bytes[offset + 1] = msg.protocol;
